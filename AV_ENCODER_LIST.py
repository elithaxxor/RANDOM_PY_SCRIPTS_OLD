import ffmpeg, sys, numpy, traceback, time, os, platform, threading, subprocess, cv2
...
def display_header():
    ...
    print(f"{'X' * 125:^70}")
    print(f"{'X' * 125:^70}")
    pretty = f'{red0}xxx AV MANIPULATOR xxx{reset0}'.center(width)
    print(f'{pretty : ^70}')
    print(f"{'X' * 125: ^70}")
...
class VideoEditor():
    def __init__(self):
        print(
            f'[+] Enter **[Desired]** File Name ex:: [sample.mp4], \n\t[+] Press Enter for default file_name [new_vid]\n')
        self.file00 = input('')
        if self.file00 == '':
            self.file00 = 'new_vid.mp4'
        self.cwd = os.getcwd()
        print(f" \n {'x' * 50} \n [++] CWD :: \n \t [{self.cwd}][++]\n")
        self.vid_loc = input('[+] Enter Video Location [Enter for CWD]')
        if self.vid_loc == '':
            self.vid_loc = self.cwd
        self.streamVideo = ffmpeg.input(self.vid_loc)
        self.path = self.vid_loc
        self.textList = os.listdir(self.path)
        self.textLen = len(self.textList)
        print(f'[+] Found [{self.textLen}] files in: \n[+]* \n [{self.path}]\n[{self.textList}]')
        self.files = []
        #### CREATE FOLDERS WITH KEY VALUES #####
        ### loading videos / images for video parser
        self.myList = os.listdir(self.path)
        self.noOfClasses = len(self.myList)
    ...
    def readList(self):
        #self.streamVideo = ffmpeg.input(self.vid_loc)
        print(f'[+]** FOUND [{self.noOfClasses}] Items')
        AV = []
        AV00 = []
        classNo = []
        classNo00 = []
        ...
        try:
            for x in range(0, (noOfClasses - 1)):
                ...
                for pics in myPicList:
                    ...
                    print('[+] AV Shape: ', AV00.shape)
                    print('[+] ClassNo Shape ', classNo00.shape)
        ...
        try:
            ...
            print(f'[+] Error Detected: -- [{errname}] \n [{err}]\n[{traceback.print_exc()}]')
            if (errname == 'OSError') and (errnum == errno.ENAMETOOLONG):
                return f'[-] Error List Name Too Long'

    def trimVideo(self):
        try:
            print('\n', 'X' * 50)
            trim_start = time.time();
            CTtime = time.ctime(trim_start)
    ...
    def encoder(self):
        try:
            ...
            ECTtime = time.ctime(encode_start)
            start = f'[+] -- [INIIATING TRIM SEQUENCE] [+] \n{ECTtime}'
            print(f'{start}')
            streamVideo = ffmpeg.output(self.streamVideo, encode_out)
            ffmpeg.run(streamVideo)
            encode_end = time.time();
            ect_end = encode_start - encode_end
...
def main():
    ...
    try:
        print(f'[+] Read ALL AV from dir?')
        readListq = input('**')
        if readListq in affirmative:
            list_len = v_Editor.textLen()
            print(f'[+] Found [{list_len}] in dir')

            counter = 0
            while counter <= range(0, list_len):
                print('[+] Initiating Iterator')
                img = v_Editor.readList()
                flag00 = v_Editor.encoder()
                print(flag00)
            print(f'[+]** Parsed [{counter}]\n [+]..Calling sys.exit')
            sys.exit(1)
    except Exception as e:
        print(f'[-] Error in Iterator  \n \n [{e}] \t\t \n{traceback.print_exc()}')
