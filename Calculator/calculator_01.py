import tkinter as Tk

calculation = ''
def add_to_calciulation(symbol):
    global calculation
    calulcation += str(symbol)
    text_result.delete(1.0, "end")
    text_result.insert(1.0, calculation)

def evaluate_calculation():
    global calculation
    try:
        result = str(eval(calculation))
        text_result.delete(1.0, "end")
        text_result.insert(1.0, calculation)

    except:
        clear_field()
        text_result.insert(1.0, "[SYS]Error")

        pass

def clear_field():
    text_result.delete(1.0, "end")
    pass


root = tk.Tk()
root.geometry("300x300")
text_result = tk.Text(root, height = 2, width = 16, font =("Ariel", 24))
text_result.grid(columnspan =5)

## Design- use lambda to avoid 'auto call' (lambda = anonymous function)
btn_1 = tk.Button(root, text="1", command = lambda : add_to_calculation(1))
btn_1.grid(row=2, column =1)

btn_2 = tk.Button(root, text="2", command = lambda : add_to_calculation(2))
btn_2.grid(row=2, column =2)

btn_3 = tk.Button(root, text="3", command = lambda : add_to_calculation(3))
btn_3.grid(row=2, column =3)

btn_4 = tk.Button(root, text="4", command = lambda : add_to_calculation(4))
btn_4.grid(row=3, column =1)

btn_5 = tk.Button(root, text="5", command = lambda : add_to_calculation(5))
btn_5.grid(row=3, column =2)

btn_6 = tk.Button(root, text="6", command = lambda : add_to_calculation(6))
btn_6.grid(row=3, column =3)

btn_7 = tk.Button(root, text="7", command = lambda : add_to_calculation(7))
btn_7.grid(row=4, column =1)

btn_8 = tk.Button(root, text="8", command = lambda : add_to_calculation(8))
btn_8.grid(row=4, column =2)

btn_9 = tk.Button(root, text="9", command = lambda : add_to_calculation(9))
btn_9.grid(row=4, column =3)

btn_0 = tk.Button(root, text="0", command = lambda : add_to_calculation(0))
btn_0.grid(row=5, column =2)

btn_plus = tk.Button(root, text="+", command = lambda : add_to_calculation(+))
btn_plus.grid(row=2, column =4)

btn_minus = tk.Button(root, text="-", command = lambda : add_to_calculation(-))
btn_minus.grid(row=2, column =4)

btn_div = tk.Button(root, text="/", command = lambda : add_to_calculation(/))
btn_div.grid(row=2, column =4)

btn_mult = tk.Button(root, text="*", command = lambda : add_to_calculation(*))
btn_mult.grid(row=2, column =4)

btn_open = tk.Button(root, text="(", command = lambda : add_to_calculation(0))
btn_open.grid(row=2, column =4)

btn_close = tk.Button(root, text=")", command = lambda : add_to_calculation())
btn_close.grid(row=5, column =3)

btn_equal = tk.Button(root, text="=", command = lambda : evaluate_calculation())
btn_equal.grid(row=6, column =3, columnspan=2 )

btn_clear = tk.Button(root, text="clr", command = clear_field, width = 12, length = 12)
btn_clear.grid(row=6, column =3, columnspan =2 )

root.mainloop()
