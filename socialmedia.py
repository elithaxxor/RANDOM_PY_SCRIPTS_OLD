...
def search(find_name):
    ...
    time.sleep(.05); print('*' * 10); time.sleep(.05); print('*' * 10, '\n'); print('[SYS] Doing my magic...'); time.sleep(.05); COUNT = 0; ACCOUNT_STATUS = True
    ...
    for url in SM.WEBSITES: ## iterate through WEBSITE list and sent get requests.
        ...
        if r.status_code == 200: # checks requerts, if 200 response then valid profile
            ...
            if SM.USERNAME in r.text: print(f'[SYS] Account - {SM.USERNAME}) - found in URL')
            else: print(f'[SYS]Account - {SM.USERNAME}) - NOT found in URL (Still a match)')
        COUNT += 1; total = len(WEBSITES) # provide dynamic counter when adding /removing websites
