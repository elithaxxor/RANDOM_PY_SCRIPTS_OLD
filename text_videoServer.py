...
'''sudo sysctl - wnet.inet.udp.maxdgram = 65535'''
...
* ATTEMMPT TO ESTABLISH SYN SYN error: (-215:Assertion failed) size.width>0 && size.height>0 in function 'imshow'-ACK CONNECTION VIA TCP, THEN SHIFT TO UDP FOR DATA TRANSFER 
* pip install PyAudio
...
port = 4921
BUFF_SIZE = 65535  # play with buffer
...
def send_vid():
    ...
    try:
        ...
        #msg = decrypt_256(msg)
        socket_address = (host_ip, int(port-3))
        ...
        while True:
            ...
            count += 1
            cv2.imshow('OUTBOUND-FRAME', frame)
...
def send_msg():
    ...
    count=0
    while True: ## to avoid broken pipe error
        if client_socket:
            while True:
                if client_socket:
                    print(f'[+] [OUTBOUND MESSAGE] [{time.ctime()}]\n [+]..TO [{addr}]')
                    data = input()
...
def recv_msg():
    ...
    client_sock, addr = recv_socket.accept()
    payload_size = struct.calcsize("Q")
    data=b''
    print(f'[+] Recv\'d msg request from [{addr}]')
...
def recv_vStream():
    try:
        BUFF_SIZE = 65536
        cv2.namedWindow('INCOMING VIDEO')
        cv2.moveWindow('INCOMING VIDEO', 400,300)
        ...
        while True:
           ...
           # data = decrypt_256(packet)  ### TESTING ---> COMMENT OUT LATER FOR FINAL TEST.
            ...
            npdata = np.fromstring(data, dtype=np.uint8) ## convert to np array for image proessing
...
def multi_thread():
    ...
    t3 = threading.Thread(target=start_stream)
