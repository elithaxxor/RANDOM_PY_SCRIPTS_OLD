...
class ImageTranslate():
    ...
    def display_Originals(self):  ### Display images
        try:
            ...
            if cv2.imshow('Result', result):
                ...
                print(f'{red}[-] Viewer Could not Open Picture {reset}'); pass
    ...
    class image_foreplay(image):
        ...
        def cascade(self):
            cascade = cv2.Canny(self.cv2_img, 125, 175); cv2.imshow('cascade', cascade); cv2.waitKey(5000)
...
except AttributeError: print(f'{red} ill fix you later--google{reset}');
pass
...
def img_draw(self, type, _boxes, _dimen, _width, _height):
    ...
    for gb in _boxes.splitlines(): print(f'[+]--{gb}'); gbSplit = gb.split(''); print(
        gbSplit)  ## creates list for easier element access
    if _boxes_len == _dimen_len: print(
        f"{yellow} [+] Succesfully parsed through boxes:: \n\t\t**[FILE] [{file}]\n\t\t {time.ctime()}")
    if cv2.waitKey(5000) & 0xFF == ord('q'): cv2.destroyAllWindows()


def translate_to_spanish(self):
    try: print(f'[+] Translating :: [SPANISH] \n [{self.inQuestion=}]'); self.spanish_translator = Translator(
        to_lang="es")
